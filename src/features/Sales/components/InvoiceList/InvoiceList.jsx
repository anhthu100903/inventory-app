...
export default function InvoiceList({ invoices = [], loading = false, onView, onEdit, onDelete }) {
  ...
  const toggleDropdown = (e, id) => {
    ...
    if (btn && btn.getBoundingClientRect) {
      ...
      setDropdownPos({ top: r.bottom + window.scrollY, left: r.left + window.scrollX, right: window.innerWidth - r.right });
    }
                  ...
                  if (isShown) {
                    ...
                    portal = createPortal(
                      <div className={styles.dropdown} style={style} onClick={(ev) => ev.stopPropagation()}>
                        <button onClick={() => handleAction("view", inv)} className={`${styles.dropdownItem} ${styles.viewItem}`}><MdInfo /> Xem chi tiết</button>
                        <button onClick={() => handleAction("edit", inv)} className={`${styles.dropdownItem} ${styles.editItem}`}><MdEdit /> Sửa</button>
                        <button onClick={() => handleAction("delete", inv)} className={`${styles.dropdownItem} ${styles.deleteItem}`}><MdDelete /> Xóa</button>
                      </div>,
                      document.body
                    );
                  }
                  ...
                  return (
                    <tr key={inv.id}>
                      ...
                      <td className={styles.actionCell}>
                        <button onClick={() => onView?.(inv)} title="Xem chi tiết" className={`${styles.actionButton} ${styles.viewButton}`}><MdInfo /></button>
                        <button onClick={() => onEdit?.(inv)} title="Sửa" className={`${styles.actionButton} ${styles.editButton}`}><MdEdit /></button>
                        <button onClick={() => onDelete?.(inv)} title="Xóa" className={`${styles.actionButton} ${styles.deleteButton}`}><MdDelete /></button>
                        <button onClick={(e) => toggleDropdown(e, inv.id)} title="Tùy chọn" className={styles.moreButton}><MdMoreVert /></button>
                        {portal}
                      </td>
                    </tr>
                  );
