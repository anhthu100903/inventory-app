...
export default function ImportForm({
  ...
  const isSubmitDisabled =
    !selectedSupplier || watchedItems.length === 0 || loading;
    ...
    >
      ...
      <div className={styles.sectionCard}>
        <div className={styles.sectionHeader}>
          ...
          <h3 className={styles.sectionTitle}>Ngày nhập hàng</h3>
        </div>
          ...
          <div className={styles.inputWrapper}>
            <input
              ...
              className={`${styles.formInput} ${
                errors.importDate ? styles.errorInput : ""
              }`}
              max={new Date().toISOString().split("T")[0]} // Giới hạn không cho chọn ngày tương lai
            />
          </div>
          {errors.importDate && (
            <p className={styles.errorMsg}>{errors.importDate.message}</p>
          )}
        </div>
      </div>
      ...
      {!showSupplierModal && (
        <div className={styles.sectionCard}>
          <div className={styles.sectionHeader}>
            ...
            <h3 className={styles.sectionTitle}>
              Sản phẩm nhập <span className={styles.required}>*</span>
            </h3>
          </div>
          <div className={styles.formGroup}>
            <ImportItemTable
            ...
            {errors.items && (
              <p className={styles.errorMsg}>
                {errors.items.message || "Ít nhất 1 sản phẩm hợp lệ"}
              </p>
            )}
          </div>
        </div>
        ...
        <div className={styles.formGroup}>
          <textarea
            {...register("note", {
              maxLength: {
                value: 500,
                message: "Ghi chú quá dài (tùy chọn 500 ký tự)",
              },
            })}
            rows={3}
            className={`${styles.formInput} ${styles.formTextarea} ${
              errors.note ? styles.errorInput : ""
            }`}
            placeholder="Ghi chú thêm về phiếu nhập (tùy chọn)..."
          />
          {errors.note && (
            <p className={styles.errorMsg}>{errors.note.message}</p>
          )}
        </div>
      </div>
